...
class MQTTHub extends Homey.App {
  async onInit() {
    try {
      ...
      Log.setLevel(
        DEBUG ? "debug" : this.settings.loglevel || DEFAULT_LOG_LEVEL
      );
  ...
  initSettings() {
    ...
    if (
      ...
      this.settings.topicRoot
    ) {
      // Backwards compatibility
      if (this.settings.topicRoot && !this.settings.homieTopic) {
        this.settings.homieTopic =
          this.settings.topicRoot +
          "/" +
          (this.settings.deviceId || systemName);
        delete this.settings.topicRoot;
      }

      this.settings.systemName = systemName;
      this.settings.deviceId =
        this.settings.deviceId || this.settings.systemName;
  ...
  async _startHomeAssistantDiscovery() {
    if (this.settings.hass) {
      ...
      this.homeAssistantDispatcher =
        this.homeAssistantDispatcher || new HomeAssistantDispatcher(this);
      await this.homeAssistantDispatcher.init(
        this.settings,
        this.deviceChanges
      );
    } else {
  ...
  async settingsChanged() {
    try {
      ...
      // devices
      if (this.deviceManager) {
        this.deviceChanges = this.deviceManager.computeChanges(
  ...
  get _birthTopic() {
    const deviceId =
      this.settings.normalize !== false
        ? normalize(this.settings.deviceId)
        : this.settings.deviceId;
    return (this.settings.birthTopic || BIRTH_TOPIC).replace(
      "{deviceId}",
      deviceId
    );
  }
  get _willTopic() {
    const deviceId =
      this.settings.normalize !== false
        ? normalize(this.settings.deviceId)
        : this.settings.deviceId;
    return (this.settings.willTopic || WILL_TOPIC).replace(
      "{deviceId}",
      deviceId
    );
  }
  async _sendBirthMessage() {
    ...
    if (this.mqttClient && this.settings.birthWill !== false) {
      ...
      return await this.mqttClient.publish(
        new Message(this._birthTopic, msg, 1, true)
      );
    }
  }
  async _sendLastWillMessage() {
    ...
    if (this.mqttClient && this.settings.birthWill !== false) {
      ...
      return await this.mqttClient.publish(
        new Message(this._willTopic, msg, 1, true)
      );
    }
  }
