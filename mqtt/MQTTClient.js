...
class MQTTClient {
  ...
  getBrokerURL() {
    ...
    Log.info("info: Broker URL = " + urlBroker.join(""));
  ...
  getConnectOptions() {
    ...
    connect_options.username = Homey.ManagerSettings.get("user");
    connect_options.password = Homey.ManagerSettings.get("password");
  ...
  _onClientAppInstalled(delay) {
    ...
    if (delay > 0) {
      Log.debug(
        `Waiting ${
          delay / 1000
        } sec. before sending messages to just started MQTT client`
      );
      this._registeredTimeout = setTimeout(
  ...
  _onClientAppUninstalled() {
    ...
    this.onUnRegistered.emit().catch((error) => Log.error(error));
  }

  _handleMessage(topic, message) {
    if (!this.registered) return;

    this.onMessage.emit(topic, message).catch((error) => Log.error(error));
  }
}
